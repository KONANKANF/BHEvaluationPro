...
public interface DepartementRepository extends AbstractBaseRepository<Departement, Long> {
	...
	@Query("select case when count(d)> 0 then true else false end from Departement d where id_departement = :directionId and lower(libelle_departement) like lower(:libelle) and isactive= 1 and is_deleted is null")
	boolean existDepartement(@Param("directionId") Long directionId, @Param("libelle") String libelleDepartement);

	@Query("select d from Departement d where id_departement = :departementId and isactive = 1 and is_deleted is null")
